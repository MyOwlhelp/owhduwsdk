...
do
	...
	local NotScriptableFixes = { --[[
		For more info: 
		- https://github.com/rbxlx/UniversalSynSaveInstance/blob/main/Tools/NotScriptable-Related/Potentially%20Missing%20Properties%20Dumper/Potentially%20Missing%20Properties%20Dumper.luau
		- https://github.com/rbxlx/UniversalSynSaveInstance/blob/main/Tools/NotScriptable-Related/NotScriptable%20Dumper/NotScriptable%20Dumper.py
		]]
		Instance = {
				...
				if Member.MemberType == "Property" then
					...
					if Serialization.CanLoad then -- If Roblox doesn't save it why should we; If Roblox doesn't load it we don't need to save it
						...
						then
							...
							if MemberTags then
								Special = table.find(MemberTags, "NotScriptable") and true
							end
...
--[=[
	...
	```lua
	local Params = {
 	RepoURL = "https://raw.githubusercontent.com/MyOwlhelp/aaaaaaaaaaa/SynX/",
...
local function dexsaveinstance(CustomOptions, CustomOptions2)
	...
	local savebuffer, savebuffer_size =
		{
			'<!-- Saved by Dex saveinstance. --><roblox version="4">',
		}, 2
	...
	do
		...
		if IsModel then
			...
			placename = (FilePath or sanitizeFileName(plceId .."mapCopy")) .. ".rbxlx"
		end
	...
	do
		...
		elseif Decompiler then
			...
			ldecompile = function()
				return "-- Executor does not have a decompiler."
			end
		end
	end
	...
	local function save_cache(final)
		...
		if StatusText then
			StatusText.Text = "Saving..(" .. get_size_format() .. ")"
		end
								...
								if PropertyName == "Source" then
									if DecompileIgnoring then -- ? Should this really prevent extraction of the original source if present ?
										...
										if should_decompile then
											...
											then
												value = "-- Impossible to decompile." -- TODO: Could be not just server scripts in the future
											else
										...
										value = "-- Saved by Dex saveinstance.\n\n"
	...
	local function save_game()
		...
		savebuffer[savebuffer_size] =
			"</roblox><!-- Saved by Dex saveinstance. -->"
		savebuffer_size += 1
		save_cache(true)
		do
	...
	if OPTIONS.ShowStatus then
		...
		StatusText.Text = string.format("Saving (%s)", get_size_format())
